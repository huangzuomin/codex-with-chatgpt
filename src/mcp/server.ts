...
import { ProjectRoutingError, WorkspaceRouter } from "../projects/router.js";
...
function mapError(error: unknown): ToolResult {
  ...
  if (error instanceof ProjectRoutingError) return fail(error.code, error.message);
...
export function createMcpServer(ctx: McpContext): McpServer {
  const { workspace } = ctx;
  const router = new WorkspaceRouter(workspace);
  const projectSelectorSchema = {
    project_id: z.string().optional().describe("Registered project id; omit for the original Bridge workspace"),
  };
  ...
  server.registerTool(
    ...
    {
      title: "Projects list",
      description: "List registered projects available to this MCP server without exposing local workspace paths.",
      inputSchema: {},
    ...
    async (_args, extra) => {
      ...
      try {
        return ok({
          projects: listProjects().map(({ id, displayName, repo, enabled }) => ({
            id,
            displayName,
            ...(repo === undefined ? {} : { repo }),
            enabled,
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      if (denied) return denied;
      try {
        const selectedWorkspace = router.resolve(args.project_id);
        const project = selectedWorkspace.detectProject();
        const git = gitInfo(selectedWorkspace.root);
        return ok({
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const selectedWorkspace = router.resolve(args.project_id);
        return ok(await selectedWorkspace.listDirectory(args.path, args));
      } catch (error) {
        return mapError(error);
      }
    }
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const selectedWorkspace = router.resolve(args.project_id);
        return ok(await selectedWorkspace.readFile(args.path, { startLine: args.start_line, endLine: args.end_line }));
      } catch (error) {
        return mapError(error);
      }
    }
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const selectedWorkspace = router.resolve(args.project_id);
        return ok(await searchWorkspace(selectedWorkspace, args));
      } catch (error) {
        return mapError(error);
      }
    }
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const selectedWorkspace = router.resolve(args.project_id);
        return ok(gitStatus(selectedWorkspace.root));
      } catch (error) {
        return mapError(error);
      }
    }
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        ...
        if (args.path) {
          relPath = selectedWorkspace.resolve(args.path).rel;
        }
        return ok(
          gitDiff(
            selectedWorkspace,
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const selectedWorkspace = router.resolve(args.project_id);
        const latest = latestExecutionRecord(selectedWorkspace.id);
        if (!latest) {
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      try {
        const selectedWorkspace = router.resolve(args.project_id);
        return ok({ records: readExecutionRecords(selectedWorkspace.id, args.limit) });
      } catch (error) {
        return mapError(error);
      }
    }
