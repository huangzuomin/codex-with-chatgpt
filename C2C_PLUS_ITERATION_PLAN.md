...
| EXECUTED(n) | 导入 DONE(n) | DONE(n) |
| EXECUTED(n) | 导入 BLOCKED(n) | BLOCKED(n) |
| BLOCKED(n) | 用户提供所需信息后恢复 | 回到阻塞前状态或显式新 PLAN |
| DONE(n) | 无 | 终态；新目标必须创建新 task |

状态迁移由 `task/lifecycle.ts` 唯一执行。Transport 无权自行改变 task 状态。
...
```json
{
  ...
  "repository": {
    ...
    "name": "repo",
    "remote": "origin",
    "branch": "c2c/c2c-f81a-runtime-contract"
  },
  "baseCommit": "full-sha",
  "headCommit": "full-sha",
...
- 使用临时文件 + rename 原子替换，避免半写状态。
- 不在 JSON 中保存 ChatGPT 全文、源码、diff、token 或凭据。
- 历史 snapshot 写入 `.c2c/tasks/<taskId>.json`；`current.json` 是当前任务副本。
...
`writeTaskSnapshot()` 在同一个应用服务中先验证 JSON，再生成 Markdown，再进行两文件原子替换。若任一生成失败，不进入 Git commit。
...
clean repo at BASE
    → 创建 task branch
    → snapshot INIT(0), baseCommit=BASE, headCommit=BASE
...
`headCommit` 不能等于“包含自身 SHA 的状态提交”，否则形成不可满足的自引用。因此采用两阶段：
...
阶段 B：状态投影提交
    → snapshot EXECUTED(n)
       ...
       changedFiles = declared changedFiles
       tests = 本轮测试结果
...
ChatGPT 的审查范围明确为 `baseCommit..headCommit`。分支最顶端还会多一个元数据提交，但不进入代码 review range。
...
2. Codex 执行最终测试。
3. 测试失败则不能进入 DONE，保持 EXECUTED 并生成新 REVIEW/PLAN 指令。
4. 测试通过后 snapshot 进入 DONE。
5. 生成状态投影提交并普通 push。
6. 不 merge、不关闭分支、不删除分支、不自动创建 PR。
...
4. 对新增和改名目标路径执行现有 `IgnoreRules.isSensitive()`。
...
- `c2c setup` 始终保留 MCP 旧语义；新的通用首次配置由 Skill + `c2c transport` / `c2c task start` 完成。
...
c2c task start <goal> [-w <workspace>] [--json]
...
3. 运行 `c2c transport set github`。
4. 运行 `c2c task start "<goal>"`。
...
EXECUTED(1) ── GitHub publish ──> CODE_HEAD + state projection
    ↓                                  ↓
REVIEW instruction                 ChatGPT reviews BASE..HEAD
    ↓                                  ↓
User copies PLAN(2) or DONE(1) <───────┘
    ├── PLAN(2) → next execution loop
    └── DONE(1) → final tests → DONE publish → summary
```
...
| `src/task/store.ts` | JSON 真源、历史 snapshot、原子写入 |
...
1. snapshot schema。
2. current.json 原子写入。
3. tasks/<id>.json 历史副本。
4. current.md 确定性投影与重建。
...
1. transport get/set。
2. task start/status/import/publish。
...
| 阶段 | 操作 | 必须保存的证据 |
| --- | --- | --- |
| INIT | `c2c task start` | taskId、branch、baseCommit、push 成功 |
...
| Review | ChatGPT 检查 BASE..HEAD | DONE 或下一轮 PLAN 文本 |
| DONE | 导入 DONE | 状态迁移校验成功 |
| Final test | 重跑完整测试 | 新鲜命令输出、0 failures |
| Final publish | 发布 DONE | 远端 current.json state=DONE |
...
| state commit 与 code head 混淆 | 两阶段提交，current.json 的 headCommit 明确定义为 code review head |
...
- JSON/Markdown 真源关系唯一且可恢复。
- `headCommit` 自引用问题通过代码提交 + 状态投影提交解决。
- 导入失败不改变 task 状态。
